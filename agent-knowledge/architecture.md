...
## Dashboard 脚本加载顺序（`index.html`，运行时契约，21 个 script）
...
→ i18n.js → ui-helpers.js → tab-url-utils.js → tab-sessions.js → session-manager.js
→ theme-controls.js → config-sync.js → drawer-manager.js → chrome-tab-groups-import.js
→ chrome-tab-groups-sync.js → dashboard-runtime.js → app.js
```
...
- `dashboard-runtime.js` 是最大的运行时中枢（约 6770 行）；`app.js` 只有 31 行，薄编排。
...
| `dashboard-runtime.js` | 打开标签卡片渲染、拖拽/多选/批量、Chrome 组卡片、会话恢复执行、事件委托 | **最大文件**，改动前先看子文档 |
...
- 全量：`node --test extension/*.test.js`（PR #42 合并后 422 pass / 0 fail）。
- 重点文件：`batch-drag.test.js`（拖拽）、`chrome-tab-groups-sync.test.js`（组同步）、`ui-regression.test.js`（回归断言含源码正则）、`tab-sessions.test.js`、`background.test.js`。
